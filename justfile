...
run:
    ...
    cloudflared tunnel run forgejo     & pid1=$!
    cloudflared tunnel run ssh-forgejo & pid2=$!

    wait $pid1 || { echo "forgejo tunnel failed"; exit 1; }
    wait $pid2 || { echo "ssh-forgejo tunnel failed"; exit 1; }
